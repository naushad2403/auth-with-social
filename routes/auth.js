...
const { EmailVerifyHTML, VerificationHTML } = require("../Utils/HtmlResponse");
...
// Registration
router.post("/register", checkExisitngUser, async (req, res) => {
  try {
    ...
    await sendEmail(
      ...
      EmailVerifyHTML(verificationLink)
    );
...
router.post("/login", async (req, res) => {
  try {
    ...
    res.json({ token, email: user.email, name: user.name , userId: user._id, role: user.role });
  } catch (error) {
